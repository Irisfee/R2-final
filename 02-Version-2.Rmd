...
ggplot(color_cor, aes(x = Group, y = Col_bia_mean, fill = Group)) +
  geom_bar(stat = "summary", fun.y = "mean", width = 0.3, alpha = .6) + # Cam: Great use of summarizing by mean inside a geom_bar!
...
p <- ggplot(d, aes(RunNo, hit_mean, group = SubjID)) +
  geom_line() + # Cam: I like the lines a bit bigger, but that is just personal preference. 
  ylab("Accuracy") +
  xlab("Run") + 
  theme(legend.position = "none") + # Cam: Nice work suppressing the legend. It was definitely not needed!
  scale_y_continuous(labels = scales::percent_format(accuracy = 1), limits = c(0, 1)) + # Cam: Moving the ylim inside scale_y_continuous should allow you to get the limits running from 0 to 100.  
  theme_minimal() 
