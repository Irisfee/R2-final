...
ggplot(color_cor_exc, aes(y = Col_bia_mean, x = `14`)) +
  geom_point(aes(colour = `14`), size = 5, alpha = .7) + # Cam: Since it looks like group is based off a mean split of some sort, I wonder whether redundant coding with `14` here may be more meaningful than splitting by group. A larger size for the points may also be helpful. The points are a bit small. 
  ylab("Color bias") +
...
d <- highlight_key(dat_dyn, ~SubjID) # Cam: I didn't know you could use the tilde to separate the highlighting by subject. In any case, nice work!
...
highlight(gg, on = "plotly_hover", dynamic = TRUE) # Cam: I never knew how to do this! Great work!
```
