...
dat_exc_asso$SubjID <- dat_exc_asso$SubjID * 10 # Cam: I'm not sure why you are multiplying the subject ids by 10
...
ggplot(color_cor_exc, aes(y = Col_bia_mean, x = `14`)) +
  geom_point(aes(colour = `14`), size = 5) + # Cam: Since it looks like group is based off a mean split of some sort, I wonder whether redundant coding with `14` here may be more meaningful than splitting by group. A larger size for the points may also be helpful. The points are a bit small. 
  ylab("Color bias") +
